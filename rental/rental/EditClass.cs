...
namespace rental
{
    class EditClass : ApplicationContext
    {
        ...
        public void AddClient(int _Id, string _FullName, string _PhoneNumber, string _Email, ClientStatus _Status)
        {
            ...
            Client client1 = new Client { Id = _Id, FullName = _FullName, PhoneNumber = _PhoneNumber, Email = _Email, Status = _Status };
            // добавляем их в бд
            Clients.Add(client1);
            SaveChanges();
        }
        ...
        {
            // создаем один объект Client
            Employee employee = new Employee { Id = _Id, FullName = _FullName, PhoneNumber = _PhoneNumber, PositionEmployeeId = _PositionEmployeeId, PositionEmployee = _positionEmployee };
            // добавляем их в бд
        ...
        {
            Movie movie = new Movie { Id = _Id, Name = _Name, GenreId = _GenreId, Genre = _Genre, Year = _Year, Country = _Country, AgeLimit = _AgeLimit, Rating = _Rating };
            // добавляем их в бд
            Movies.Add(movie);
            SaveChanges();
        }
        ...
        {
            Disk disk = new Disk { Id = _Id, Status = _Status, Movies = _Movies };
            SaveChanges();
        }
        ...
        {
            Order order = new Order { Ordernumber = _Ordernumber, Cost = _Cost, Date = _Date, ReturnDate = _ReturnDate, Pledgetype = _Pledgetype, Status = _Status, Disks = _Disks };
            SaveChanges();
        }
